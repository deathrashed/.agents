---
name: egc-save-session
description: Save Session Command
---
---
description: Save current session state to a dated file in ~/.gemini/egc-sessions/ so work can be resumed in a future session with full context.
---
...
```bash
mkdir -p ~/.gemini/egc-sessions
```

### Step 3: Write the session file

Create `~/.gemini/egc-sessions/YYYY-MM-DD-<short-id>-session.tmp`, using today's actual date and a short-id that satisfies the rules enforced by `SESSION_FILENAME_REGEX` in `session-manager.js`:
...
- The file is meant to be read by Gemini at the start of the next session via `/egc-resume-session`
- Use the canonical global session store: `~/.gemini/egc-sessions/`
