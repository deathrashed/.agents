...
Manage Gemini Code session history - list, load, alias, and edit sessions stored in `~/.gemini/egc-sessions/`.
...
Use `/egc-sessions info` when you need operator-surface context for a swarm: branch, worktree path, and session recency.
...
- Session files persist `Project`, `Branch`, and `Worktree` in the header so `/egc-sessions info` can disambiguate parallel tmux/worktree runs.
- For command-center style monitoring, combine `/egc-sessions info`, `git diff --stat`, and the cost metrics emitted by `scripts/hooks/cost-tracker.js`.
