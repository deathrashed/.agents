---
name: egc-prompt-optimize
description: /egc-prompt-optimize
---
---
...
0. **Project Detection** — Read GEMINI.md, detect tech stack from project files (package.json, go.mod, pyproject.toml, etc.)
...
If the user asks for direct execution, explain that `/egc-prompt-optimize` only produces advisory output and tell them to start a normal task request instead.
