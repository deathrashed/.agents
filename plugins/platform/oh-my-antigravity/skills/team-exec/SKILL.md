...
2. Enforce execution preconditions:
   ...
   - if either is missing, stop this slice as `blocked` and route to `/oma:team-plan` or `/oma:team-prd` first
...
4. Before editing, confirm the lane's expected baseline branch or HEAD anchor when known:
   ...
   - recommend `/oma:workspace audit`, `/oma:team-plan`, or explicit branch realignment before continuing
...
7. Delegate implementation to `oma-executor` with explicit lane/subagent context.
...
8. If the requested implementation agent is unavailable (for example "agent not found"), retry once with fallback routing:
   - low-risk slice -> `oma-quick`
   - architecture-sensitive or high-risk slice -> `oma-director` for reassignment before retry
...
13. If lane health is unsafe for review or automation, call it out and recommend `/oma:workspace audit` or isolated worktree use.
