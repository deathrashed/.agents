---
name: doctor
description: Run OmG diagnostics for extension readiness, workflow state integrity, and multi-agent safety.
---
...
2. Verify extension surface integrity:
   - key directories (`agents`, `commands/omg`, `skills`, `context`)
   - key manifest/context files (`gemini-extension.json`, `GEMINI.md`, `context/omg-core.md`)
...
4. Verify retained deep-work skills (`plan`, `omg-plan`, `execute`, `prd`, `ralplan`, `research`, `deep-dive`, `context-optimize`) and their metadata integrity.
...
6. Detect common failure patterns:
   ...
   - Gemini CLI runtime too old for current OmG assumptions (recommend `v0.38.0+`)
   - missing or stale root `GEMINI.md` import chain
   - mixed or stale OmG extension roots (for example: copied assets, duplicate local clones, or stale installed extension contents diverging from the active repository)
   - command discovery mismatch
   - manual hook or skill wiring shadowing extension-managed OmG assets
   ...
   - duplicate OmG hook registration causing repeated AfterAgent output or mixed manual/extension hook paths
...
8. Prefer extension-managed recovery paths first:
   ...
   - keep one authoritative OmG hook registration path per event before editing shipped assets
