...
5. For non-trivial implementation work, treat `/omg:workspace set ...` or existing workspace state plus `/omg:workspace audit` as the default preflight before execution.
6. Recommend `/omg:taskboard sync` after audit whenever the task spans multiple roots, parallel lanes, or any resume/handoff boundary.
