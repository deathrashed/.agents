...
wf(){ rt="$1"; [ ! -L "$rt/memory" ]||return 1; bf "$rt/memory/wiki" "$2"; }
...
$ex"; added=1; }; fi
    project="$(awk 'NR==1&&$0=="---"{i=1;next}i&&$0=="---"{exit}i&&/^project:/{sub(/^project:[[:space:]]*/,"");gsub(/["'\''[:space:]]/,"");print;exit}' "$hot" 2>/dev/null || true)"
    wiki="$(wf "$rt" "index.md" || true)"
    if printf "%s" "$project"|grep -Eq '^[A-Za-z0-9][A-Za-z0-9_-]{0,63}$'; then c="$(wf "$rt" "$project/index.md" || true)"; [ -n "$c" ] && [ -f "$c" ] && wiki="$c"; fi
    if [ -f "$wiki" ] && [ ! -L "$wiki" ]; then su="$(sr "$wiki" 80 12000)"; [ -n "$su" ] && { body="$body
...
$su"; added=1; }; fi
  ...
  post-tool-use)
    ...
    case "$rel" in
      ...
      memory/*|hooks/*|commands/*|references/*|marketplaces/*|scripts/*|*.json|*.yml|*.yaml|*.cff|*SKILL.md|CLAUDE.md|README.md|docs/*) exit 0;;
